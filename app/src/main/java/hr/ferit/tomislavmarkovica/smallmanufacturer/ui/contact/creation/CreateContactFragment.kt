...
class CreateContactFragment: Fragment() {
    ...
    private fun getContactFromInput(): Contact {
        ...
        return Contact(id = 0 ,firstName = firstName, lastName = lastName, email = email, phoneNumber = phoneNumber, address = address)
    }

    private fun saveContact() {
        viewModel.saveContact(getContactFromInput())
        //viewModelFeatures.save(productId)

        Toast.makeText(context, "New contact added", Toast.LENGTH_SHORT).show()
        Navigation.findNavController(binding.root).navigate(R.id.action_createContactFragment_to_holderFragment)
    }
}